...
-- slightly. Unfortunately, this cannot be used in a modeline. Instead, it can
-- be enabled by putting the custom marker (see below) at the start or end of
-- the HTML file.

local M = {}

M.marker = "VIM_CUSTOM_HTML_SLIDES_FOLDING"
...
function M.foldtext()
  ...
  return string.format("▶ %s  [%d lines]", title, (e - s + 1))
end
...
vim.api.nvim_create_autocmd("BufReadPost", {
  ...
  callback = function(args)
    ...
    local head = table.concat(
      vim.api.nvim_buf_get_lines(args.buf, 0, math.min(10, n), false), "\n")
    local tail = table.concat(
      vim.api.nvim_buf_get_lines(args.buf, math.max(n - 10, 0), n, false), "\n")
    local marker = my_htmlslides.marker
    if head:find(marker, 1, true) or tail:find(marker, 1, true) then
      vim.opt_local.foldmethod = "expr"
      vim.opt_local.foldexpr  = "luaeval('my_htmlslides.foldexpr()')"
      vim.opt_local.foldtext  = "luaeval('my_htmlslides.foldtext()')"
    end
  end,
})
